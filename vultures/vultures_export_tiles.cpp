...
void save_tile(
    int tile_enum, vultures_tile *tile,
    int offset_x, int offset_y,
    int max_width, int max_height
  )
{
  ...
  filename = tilename_reference[tile_enum] + ".png";
  ...
  printf("Saving %s (%d,%d)\n", filename.c_str(), toRect.x, toRect.y);
...
void vultures_export_tiles()
{
  ...
  for ( mon_id=0 ; mon_id < NUMMONS ; mon_id++ )
      recalculate_bounds( vultures_get_tile( MONSTER_TO_VTILE(mon_id) ), offset_x, offset_y, max_width, max_height );

  for ( obj_id=0 ; obj_id < NUM_OBJECTS ; obj_id++ )
      recalculate_bounds( vultures_get_tile( OBJECT_TO_VTILE(obj_id) ), offset_x, offset_y, max_width, max_height );

  /* Save each monster's tile */
  for ( mon_id=0 ; mon_id < NUMMONS ; mon_id++ )
      if (MONSTER_TO_VTILE(mon_id) != V_TILE_NONE)
          save_tile( glyph2tile[ monnum_to_glyph( mon_id ) ], vultures_get_tile( MONSTER_TO_VTILE(mon_id) ), offset_x, offset_y, max_width, max_height );

  /* Save each object's tile */
  for ( obj_id=0 ; obj_id < NUM_OBJECTS ; obj_id++ )
      if (OBJECT_TO_VTILE(obj_id) != V_TILE_NONE)
          save_tile( glyph2tile[ objnum_to_glyph( obj_id ) ], vultures_get_tile( OBJECT_TO_VTILE(obj_id) ), offset_x, offset_y, max_width, max_height );
